...
sales %>% dplyr::filter(region == '01-East')%>%select(partnum, region)%>% head
sales %>% dplyr::filter(region == '01-East' | revenue > 400000)%>%select(partnum, region)%>% head
sales %>% dplyr::filter(region == '01-East' | revenue > 400000)%>%select(partnum, region)
sales %>% dplyr::filter(region == '01-East' & revenue > 400000)%>%select(partnum, region)
names(sales)
